...
export async function resolvePageWriteTarget(
  ...
  if (sourceLocalPath) {
    ...
    // Fork carry (originally d0a4cc16): the `${slug}.md` fallback below only
    // fires when there is no recorded `source_path` — a page born via
    // put/capture under a `config.slug_prefix` source, never synced from a
    // file. The STORED slug there is `${prefix}/<repo-relative path>`, but a
    // file at that path would live INSIDE the synced tree, and the next full
    // sync would re-import it under a DOUBLED prefix
    // (`news-server/news-server/agents/foo`) — silently, once per
    // put+commit+sync cycle. Strip the mount point so the fallback mirror
    // still round-trips. No effect when recordedPath exists (the common
    // case) — that branch already resolves the correct on-disk path.
    ...
    filePath = recordedPath
      ...
      ? resolveSourceLocalFilePath(sourceLocalPath, recordedPath, repoRelSlug) ?? join(sourceLocalPath, `${repoRelSlug}.md`)
      : join(sourceLocalPath, recordedPathFromFileUri(recordedUri, sourceLocalPath) ?? `${repoRelSlug}.md`); // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal -- result passes isWriteTargetContained before any write (#4204/#4289 guard)
...
export async function writePageThrough(
  ...
  try {
    ...
    // #2018 leak guard, recorded-source_path preference, and the fork's
    // slug_prefix mount stripping all live in the shared helper now —
    // resolvePageWriteTarget is also used by the facts fence writer, so both
    // MUST compute the identical path (see its doc comment above).
