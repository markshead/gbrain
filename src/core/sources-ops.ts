...
export interface AddSourceOpts {
  ...
  force?: boolean;
  /**
   * v0.42.x per-source slug namespacing (fork carry, originally d0a4cc16):
   * stored as `config.slug_prefix`. Every page synced/imported from this
   * source mounts under `${slugPrefix}/` (e.g. prefix "news-server" →
   * slug "news-server/agents"). Must match SLUG_PREFIX_RE (lowercase
   * alnum-hyphen segments, single slashes, no leading/trailing slash);
   * invalid values are rejected loudly.
   *
   * Choose it HERE, at registration, and treat it as immutable: adding or
   * changing a prefix on a source that already has pages forks their slugs
   * (stored slugs resolve through pages.source_path and are never
   * re-prefixed, while re-imports land on the new prefixed slug). `sync`
   * warns when it detects that drift; there is deliberately no mutation
   * surface for the prefix.
   */
  slugPrefix?: string;
  /**
...
export async function addSource(
  ...
  // v0.42.x (fork carry): validate the slug prefix BEFORE any clone/DB work
  // so a bad value never lands in sources.config (sync fails loudly on
  // invalid config, so rejecting at registration is the friendlier surface).
  ...
  if (parsedUrl) {
    ...
    if (attachPath) {
      ...
      // v0.42.x (fork carry): immutable at registration only — the attach
      // branch above (existing path-less row) deliberately does not accept
      // a prefix change, per the "no mutation surface" contract on
      // AddSourceOpts.slugPrefix.
      if (opts.slugPrefix !== undefined) {
        config.slug_prefix = opts.slugPrefix;
      }
      const displayName = opts.name ?? opts.id;
