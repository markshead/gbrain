...
const list_pages: Operation = {
  ...
  params: {
    ...
    // Fork carry (originally #3812, closed upstream for lack of demonstrated
    // demand rather than a quality objection — see infra/gbrain-fork-upstream-decision
    // in gbx). Both engines have carried PageFilters.slugPrefix all along
    // (search below); this op just never declared/forwarded it. Plain
    // string-prefix semantics, same LIKE-metachar-escaped range scan
    // get_recent_salience's slugPrefix param already uses.
    slug_prefix: {
      type: 'string',
      description:
        "Filter to slugs starting with this prefix, e.g. 'wiki/people/'. Include the trailing " +
        "'/' to stay inside a path segment ('wiki/people' also matches 'wiki/peoples-court').",
    },
