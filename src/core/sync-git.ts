 ...
 *
 * v0.42.x (fork carry): `slugPrefix` (the source's `config.slug_prefix`, when
 * set) applies ONLY to the derived fallback — a stored slug already carries
 * the prefix from import time, so re-prefixing it would double-apply.
 */
...
export async function resolveSlugsForRemovedPaths(
  ...
  // v0.42.x (fork carry): threaded into the phase-2 fallback derivation only
  // — an exact source_path hit (phase 1) already carries the prefix from
  // import time and must never be re-prefixed.
  slugPrefix?: string,
  ...
  for (const p of paths) {
    ...
    else fallbacks.push({ path: p, slug: resolveSlugForPath(p, slugPrefix) });
  }
...
export async function resolveRemovedPathSlug(
  ...
  const res = await resolveSlugsForRemovedPaths(engine, [path], sourceId, slugPrefix);
