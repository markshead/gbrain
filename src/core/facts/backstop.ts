  ...
  return runPipelineWithBody(
    {
      ...
      ref: parsedPage.slug,
      // 2026-09-02 (local patch): explicit page provenance for the legacy
      // DB-only bucket. Deliberately NOT reusing `ref` — that field is
      // overloaded (page slug here, sessionId on the turn-text entry), so
      // reading it downstream would stamp session ids into
...
async function runPipelineWithBody(
  ...
  for (const { f, resolvedSlug } of legacyBucket) {
    const newFact: NewFact = {
      ...
      // 2026-09-02 (local patch): without this, a brain with no
      // sources.local_path loses page provenance on EVERY fact.
